 ...
 *      2. 就绪状态：指该线程已经被创建（与操作系统线程关联），可以由 CPU 调度执行
 ...
 *
 * 从JavaAPI层面（Thread.State），线程有六种状态：
 *      1. NEW: 线程刚被创建，还没调用start方法
 *      2. RUNNABLE：调用了start方法之后，此状态覆盖了操作系统层面的"就绪状态"，"运行状态"，"阻塞状态"（由代码执行过程中导致的阻塞，java无法识别，仍认为是可运行的）
 *      3. BLOCKED：未获取到锁时的阻塞状态
 *      4. WAITING：阻塞状态
 *      5. TIMED_WAITING：有时间限制的阻塞状态
